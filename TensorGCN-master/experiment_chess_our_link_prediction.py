...
for tr in range(no_trials):
	for alpha in alpha_vec:
		...
		if no_layers == 2: 
			gcn = ehf.EmbeddingGCN2(Ct_train_2[:-1], X_train[:-1], e_train, M[:-1, :-1], hidden_feat=[6,6,2], condensed_W=True, use_Minv=False, nonlin2="selu")
		elif no_layers == 1:
			gcn = ehf.EmbeddingGCN(Ct_train_2[:-1], X_train[:-1], e_train, M[:-1, :-1], hidden_feat=[6,2], condensed_W=True, use_Minv=False)
