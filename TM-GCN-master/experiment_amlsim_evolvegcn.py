# This version of amlsim experiment imports data preprocessed in Matlab, and uses EvolveGCN
...
for tr in range(no_trials):
	for alpha in alpha_vec:
		...
		save_res_fname = "results_EVOLVEGCN_layers" + str(no_layers) + "_w" + str(round(float(class_weights[0])*100)) + "_" + dataset

		# Create gcn for training
		if no_layers == 2:
			gcn = ef.EvolveGCN_2_layer(C_train, X_train, edges_train, [6,6,2])
		elif no_layers == 1:
			gcn = ef.EvolveGCN_1_layer(C_train, X_train, edges_train, [6,2])
		...
		for ep in range(no_epochs):
			...
			with t.no_grad():
				...
				if ep % 100 == 0:
					...
					if no_layers == 2:
						output_val, W_test, W2_test = gcn(C_val, X_val, edges_val, W_val, W2_val)
					elif no_layers == 1:
						output_val, W_test = gcn(C_val, X_val, edges_val, W_val)
					...
					if no_layers == 2:
						output_test, _, _ = gcn(C_test, X_test, edges_test, W_test, W2_test)
					elif no_layers == 1:
						output_test, _ = gcn(C_test, X_test, edges_test, W_test)
